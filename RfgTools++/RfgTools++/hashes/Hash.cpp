...
//From gibbed.volition repo. See license in Hash.h
u32 _CrcVolitionTable[256] =
{
...
namespace Hash
{
    u32 HashVolitionCRC(const string& input, u32 hash)
    {
        ...
        {
            hash = _CrcVolitionTable[(u8)hash ^ (u8)inputLowercase[i]] ^ (hash >> 8);
        }
        return hash;
    }
    ...
    */
    ...
    u32 HashVolition(const string& input)
    {
        ...
        for (int i = 0; i < inputLowercase.size(); i++)
        {
            ...
            hash = (char)(inputLowercase[i]) ^ hash;
        }
        return hash;
    }
}